...
// ✅ Reusable function for absentee notification logic
async function generateAbsenteeNotifications() {
  ...
  return rows.length; // count of inserted notifications
}

// ✅ Cron job runs every day at 9:00 AM
cron.schedule('0 9 * * *', async () => {
  try {
    const count = await generateAbsenteeNotifications();
    console.log(`Absentee notifications generated at 9:00 AM. Count: ${count}`);
  } catch (err) {
    console.error('Cron error while generating absentee notifications:', err);
  }
});

// ✅ Optional: API endpoint to trigger manually if needed
app.post('/api/generate-absentee-notifications', async (req, res) => {
  try {
    const count = await generateAbsenteeNotifications();
    res.json({ message: 'Absentee notifications generated.', count });
  } catch (err) {
